...
void Game::Menu()
{
	...
	rightarrow.setPosition(OptionButton.getGlobalBounds().left + OptionButton.getGlobalBounds().width + 15.0f, mode.height / 2.0f + OptionButton.getGlobalBounds().height / 4.0f);// set vị trí của icon canh theo vị trí nút option
	...
	leftarrow.setPosition(OptionButton.getGlobalBounds().left - 15.0f, mode.height / 2.0f + OptionButton.getGlobalBounds().height / 4.0f);// set vị trí của icon canh theo vị trí nút option
	...
	while (gamestate == GameState::Menu)
	{
		...
		while (window.pollEvent(event)) {
			...
			case Event::KeyPressed: // xử lý khi nhấn nút trên bàn phím
			{
				...
				if (Keyboard::isKeyPressed(Keyboard::Right) && CommandOrder < 3)
				{

					audiomanager.PlaySound("transitionbutton.wav", false);
					CommandOrder++;// tăng lựa chọn lệnh 1 đơn vị mỗi khi nhấn nút
				...
				else if (Keyboard::isKeyPressed(Keyboard::Left) && CommandOrder > 1)
				{
					...
					audiomanager.PlaySound("transitionbutton.wav", false);
					CommandOrder--;// giảm lựa chọn lệnh 1 đơn vị mỗi khi nhấn nút
		...
		BgBall.Update(DeltaTime, mode, audiomanager, player1, player2, playground, Scale); // Update vị trí quả banh theo thời gian giữa 2 khung hình 
	...
	while (gamestate == GameState::PlayerMode) {
		...
		while (window.pollEvent(event))
		{
			...
			case Event::KeyPressed: // xử lý sự kiện khi có nút bàn phím được bấm 
			{
				...
				// xử lý khi nút backspace đucợ nhấn 
				if (Keyboard::isKeyPressed(Keyboard::BackSpace))
				{
					audiomanager.PlaySound("enterbutton.wav", false);
...
void Game::RunGame()
{
	...
	//Tạo các object cần cho 1 game đấu gồm: 2 người chơi, sân chơi 
	PlayGround playground(mode, DisplayFont, Scale); // sân chơi được tạo dựa vào constructor mode, font và tỉ lệ căn chỉnh độ phân giải
	Player player1(mode, PlayerMode::player1, Scale);// tạo 2 player , với chế độ chơi mặc định là 2 người chơi người 1 và 2
	Player player2(mode, PlayerMode::player2, Scale);
	if (playmode == PlayerMode::Computer) // nếu là chế độ chơi là máy thì sẽ set lại chế độ chơi là máy 
	{
		player2.ChangeMode(PlayerMode::Computer);
	}
	Ball ball(mode, Scale);
	...
	while (gamestate == GameState::Playing)
	{
		...
		// Hàm sử dụng để điều khiển 2 player
			player1.MovementControl(playground.GetList().at(0), playground.GetList().at(2), DeltaTime, ball, difficult, Scale);

			player2.MovementControl(playground.GetList().at(0), playground.GetList().at(2), DeltaTime, ball, difficult, Scale);

	
			ball.Update(DeltaTime, mode, audiomanager, player1, player2, playground, Scale);// cập nhật vị trí banh
		}
		...
		window.draw(playground.Score[0]);
		window.draw(playground.Score[1]);
		...
		if (player1.GetScore() == WinScore || player2.GetScore() == WinScore) // kiểm tra xem người chơi nào đạt tới WinScore trước
		{
...
void Game::RunPauseMenu()
{
	...
	if (gamestate == GameState::Pause)
	{
		...
		// cài đặt phông nền 
		Background.setSize(Vector2f(mode.width / 2.5f, mode.height * 3.0f / 4.0f));
		Background.setOrigin(Background.getLocalBounds().width / 2.0f, Background.getLocalBounds().height / 2.0f);
		Background.setFillColor(Color(255, 175, 176, 250));
		Background.setOutlineColor(Color::White);
		Background.setOutlineThickness(1.0f);
		...
		MainMenuButton.setPosition(mode.width / 2.0f, mode.height * 1.25f / 2.0f);
		...
		QuitButton.setPosition(mode.width / 2.0f, mode.height * 1.5f / 2.0f);
		CommandOrder = 1;
			...
			while (window.pollEvent(event))
			{
				...
				case Event::KeyPressed:
				{
					...
					else if (Keyboard::isKeyPressed(Keyboard::Enter))
					{
						...
						// xử lý khi lựa chọn trong Menu tạm dừng đang là 2 ( nút trở về Menu chính) và biến tạm dừng đang là true
						if (CommandOrder == 2)
						{
						...
						// xử lý khi lựa chọn trong Menu tạm dừng đang là 3 ( nút thoát game Quit) và biến tạm dừng đang là true
						if (CommandOrder == 3)
						{
